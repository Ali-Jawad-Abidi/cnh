...
var CLIENT_ID =
  "ErA7RBnKLK4ThvTq7E2dZKoThh3ptjr5FWyJ1a_fDxNYyJQCNaaHNX6HuZaGrneW";
var CLIENT_SECRET =
  "IMT5Bvo03hC2FneoecBinbzw33hRyTlXJQrj0zAdOiMn5JRH4eiRFyM68ui4UJk0";
          ...
          if (err) {
            ...
            userModel.find({ _id: req.body.addedby }, (err, data) => {
              if (err) {
                ...
                const response = con.save((err, data) => {
                  if (err) {
                    res.status(400).json({
                      errorMessage: err,
  ...
  if (req.query.id) {
    brandModel
      .find({ _id: req.query.id }, (err, ret) => {
        if (err) res.status(200).json({ status: false, msg: err });
        else {
          subcatsModel.find(
            {
              _id: {
      ...
      userModel.find({ username: req.body.username }, (err, data) => {
        if (data.length > 0) {
          if (bcrypt.compareSync(req.body.password, data[0].password)) {
            ...
            checkUserAndGenerateToken(data[0], req, res, false);
          } else {
            res.status(200).json({
              ...
              if (err) {
                ...
                User.save((err, data) => {
                  if (err) {
                    ...
                    checkUserAndGenerateToken(User, req, res, true);
                  }
                });
              }
            });
...
function checkUserAndGenerateToken(data, req, res, sendMail) {
  jwt.sign(
...
app.post("/api/loginfacebook", (req, res) => {
  if (
    ...
    User.save((err, data) => {
      checkUserAndGenerateToken(User, req, res, false);
    });
  } else {
    res.status(400).send("Error in request");
  }
...
app.get("/api/removemerchcat", (req, res) => {
  if (req.query.id) {
    MerchCatModel.find({ _id: req.query.id }).deleteOne();
    res.status(200).send("Success");
  } else {
    res.status(400).send("Bad Request");
  }
              ...
              orderModel.find(
                ...
                (err, ret) => {
                  ...
                  var toSend = {
                    ...
                    email: data[0].email,
                    bitshistory: data[0].bitshistory,
                  };
...
app.post("/api/getstats", async (req, res) => {
  ...
  promises.push(orderModel.find({}).count());
  promises.push(merchModel.find({}).count());
  promises.push(userModel.find({}).count());
  promises.push(commentModel.find({}).count());
  promises.push(brandModel.find({}).count());
  promises.push(consoleModel.find({}).count());
  promises.push(blogModel.find({}).count());
  promises.push(tokenModel.find({}).count());

  Promise.all(promises).then((response) => {
    stats.orders = response[0];
    stats.products = response[1];
    stats.users = response[2];
    stats.comments = response[3];
    stats.brands = response[4];
    stats.consoles = response[5];
    stats.blogs = response[6];
    stats.loggedInUsers = response[7];
